...
$ ->
  ...
  ###
  The server-side template puts the type-safe URL into that field. That way my static assets remain static assets.
  But my server-side template still have the type-safety in them.

  Static assets should be static. If you mix server-side templating with static assets then they become dynamic assets.
  That is why you shouldn’t mix the two.
  ###
