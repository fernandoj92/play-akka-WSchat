...
class Chat @Inject()(val messagesApi: MessagesApi, system: ActorSystem) extends Controller with I18nSupport {
  ...
  //New
  def chatList = Action { implicit request =>
    request.session.get(User).map { user => Ok(views.html.chatList())
    }.getOrElse(Redirect(routes.Chat.index()))
  }
  //New
  def chat(name :String) = Action { implicit request =>
    request.session.get(User).map { user => Ok(views.html.chatNew(user,name))
    }.getOrElse(Redirect(routes.Chat.index()))
  }

  def chat = Action { implicit request =>
    request.session.get(User).map { user => Ok(views.html.chat(user))
    }.getOrElse(Redirect(routes.Chat.index()))
  }
  ...
  //One websocket per person
  //One socket, but multiple rooms (typed messages)
